...
def draw_dilate(mouse_x, mouse_y, sq_size):
    ...
    dilated_img = bounded_img

    if dilate_index <= max_dilate:
        pass
    else:
        # dilate
        if max_dilate < 0:
            dilate_index = max(abs(max_dilate), dilate_index)
            dilated_img = cv2.erode(dilated_img, kernel, iterations=dilate_index)
            dilate_index -= 1
        else:
            if dilate_index >= 0:
                dilated_img = dilate3(dilated_img, dilate_index, 128 - 2 * dilate_index,
                                      step=sq_size // 3)
                dilate_index -= 1
            else:
                # print(dilate_index, max_dilate)
                dilate_index = max(0 - max_dilate, dilate_index)
                dilated_img = cv2.dilate(dilated_img, kernel, iterations=abs(dilate_index))
                dilate_index -= 3
...
max_sq_size = int(canvas_width // 5 * 8)
init_sq_size = 10
poly_n = 15
