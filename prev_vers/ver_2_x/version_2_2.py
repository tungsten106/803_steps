...
"""
version2.2: 添加了产生quadtree的功能，尚未完善
在里面不画格子，在外面画
"""
...
def draw_dilate(mouse_x, mouse_y, sq_size):
    ...
    # mask.show()
    screenshot = np.array(cropped_sub_small)

    # tmp = Image.new('L', (max_sq_size, max_sq_size), color=0)
    # tmp_draw = ImageDraw.Draw(tmp)
    # # draw.rectangle((x, y, x + width, y + height), fill="white")
    # tmp_draw.polygon(new_polygon.vertices, fill="white")
    # matrix = crop_polygon(np.array(tmp),
    #                       new_polygon)
...
def on_mouse_press(event):
    ...
    # 设置蒙版
    # mask = Image.new('L', (window.winfo_height(), window.winfo_width()), color=0)
    mask = Image.new('L', (canvas_width, canvas_width), color=0)
    draw = ImageDraw.Draw(mask)
    # draw.rectangle((x, y, x + width, y + height), fill="white")
    draw.polygon(old_polygon.moved_vertices(x, y), fill="white")
    draw.polygon(old_polygon.moved_vertices(x*2, y*2), fill="white")
    # mask.show()
    ...
    first_square = square(0, 0, row - 1, col - 1)
    square_arr = [first_square]
    square_arr = get_quad_tree(matrix, square_arr, min_sq=max_sq_size//2)

    mask2 = Image.new('L', (canvas_height, canvas_width), color=0)
    # mask2 = Image.new('L', (window.winfo_height(), window.winfo_width()), color=0)
    ...
    for i in square_arr:
        ...
        for row in range(i.lx, i.rx):
            mask2[row][i.ry] = 255
    # Image.fromarray(screenshot).show()
    screenshot_photo = ImageTk.PhotoImage(Image.fromarray(mask2))
    canvas.create_image(0, 0, anchor=tk.NW, image=screenshot_photo, tags="revealed_mask")
    canvas.screenshot_photo = screenshot_photo
