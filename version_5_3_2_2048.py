...
coord_list = []
for i in range(50):
    xi = 300 + np.random.uniform(-100, 100)
    yi = 500 + np.random.uniform(-100, 100)
    coord_list.append((
        xi, yi
    ))
...
while True:
    ...
    # gen new coord
    pressed_x = 300 + np.random.uniform(-200, 200)
    pressed_y = 500 + np.random.uniform(-200, 200)
    if 0<=pressed_x<canvas_width and 0<= pressed_y<canvas_height:
        coord_list.append((pressed_x, pressed_y))
